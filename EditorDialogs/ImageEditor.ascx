...
//==================================== Advanced TODO List ========================================================//
/*
//Features that likely wont be implemented due to lack of time and will need to remain for next time
- Rotation by angle (will be possible to implement at later stage in both IE and FF, but it is of little use now, and a lot of work to do properly
*/

//============================ /TEMP Initialization function ======================================================//

	//Integrate code with RadSlider
...
/*
	- Mozilla Support!
	- Add move support!
	- Real time values display when resizing
	- Removed the TabStrip - all is now in one pane
	//5. TODO: Convert the Draggable object to an extender or sth
	- Attach onchange handlers to the UI to change the EDITABLE IMAGE
	- Update the current values for the image when switching tabs
	- ImageObject with API that will capsulate the functionality in a transparent manner and hide differences for IE and Moz
	- Detect change in the tab
	- Added reset button
	- Hook all props to a the UI
	- Have the image load in its area
	- Add event to notify parent
	- Add persentages/points (thumbnail creator)
	- Add lock ratio (image props)
	- Suffix property
	- Events on the client
	- MVC archtiecture - parent (ImageManager), child (ImageEditor)
	- Implement a callback function from the AJAX, notify the parent image dialog. The event should be made to reload the current directory, and to select the image
* --%>
...
Telerik.Web.UI.Widgets.ImageEditor.prototype = {
	...
	clientInit : function(clientParameters)
	{
		...
		this.set_src(this._clientParameters.imageSrc);
		if (this._clientParameters.suffix)
		{
			this.set_suffix(this._clientParameters.suffix);
		}
	...
	setupChildren : function()
	{
		...
		this._opacitySlider.add_valueChange(Function.createDelegate(this, this._onOpacitySliderChanged));

		this._widthSlider = $find("widthSlider");
		this._widthSlider.add_valueChange(Function.createDelegate(this, this._onSizeSliderChanged));

		this._heightSlider = $find("heightSlider");
		this._heightSlider.add_valueChange(Function.createDelegate(this, this._onSizeSliderChanged));
		...
		this._toggleCrop.title = this._cropEnabled ? localization["EnableCrop"] : localization["DisableCrop"];
	},
	...
	_enableHtmlElements : function(toEnable)
	{
	...
	enableCrop : function(toEnable)
	{
		...
		this._enableHtmlElements(toEnable);

		//Change button text
		//this does not work at the moment because of the form decorator
		//this._toggleCrop.innerHTML = "<span>" + (this._cropEnabled ? localization["DisableCrop"] : localization["EnableCrop"]) + "</span>";
		//this._toggleCrop.title = this._cropEnabled ? localization["DisableCrop"] : localization["EnableCrop"];
	...
	_updateControllerUI : function()
	{
		...
		//Opacity - TODO: Update the slider
	...
	_updateControllerCrop : function(bounds)
	{
		if (!bounds) bounds = {};
   ...
   //======================================= MORE API ===================================================//
	...
	_resetImageEditor : function()
	{
		...
		var nameInput =  this._newImageNameControl;
		var suffix = this.get_suffix();
		var dotPos = src.lastIndexOf(".");
		var newName = src.substring(0,dotPos) + suffix + src.substring(dotPos);
	...
	{
		var name = this.get_newImageName();
		if (!name)
		{
	...
	get_src : function()
	{
		...
		return this._originalImageLocationControl.value;
	},

	set_src : function(src)
	{
		//Set src in the hidden field
		this._originalImageLocationControl.value = src;
	},
...
Telerik.Web.UI.Widgets.DraggableResizer.prototype =
{
	...
	_createUI : function()
	{
	   ...
	   tableElement.className = "rade_draggableResizer";
	...
	{
		this._cropElement.style.display = visible ? "" : "none";
	},
...
<style type="text/css">
	...
	.rade_draggableResizer .resizerHandle
	{
		background: #cc0000;
		width: 6px !important;
...
<table id="dialogControl" cellpadding="0" cellspacing="0" class="rade_dialog ImageEditor">
	<tr>
		<td style="width: 470px; vertical-align: middle; text-align:center;">
				<div id="ImageEditorPane" class="radEImagePreview">&nbsp;</div>
		</td>
		<td style="padding-left: 4px; vertical-align: top;">
					...
					<tr>
						...
						<td>
							<div style="margin-top: 4px;">
								<telerik:RadSlider ID="opacitySlider" runat="server" Length="200" MinimumValue="0"
									MaximumValue="100" Value="100" SlideStep="1" />
							</div>
						</td>
						<td>
							<input id="opacityInput" value="100" type="text" readonly="readonly" style="width: 40px;
								text-align: right;" />
							%
						</td>
					</tr>
			...
			<fieldset style="width: 322px">
				...
				<table border="0" cellpadding="0" cellspacing="0" class="FlipTable">
					<tr>
						...
						<td>
							<div style="margin-top: 4px;">
								<telerik:RadSlider ID="widthSlider" runat="server" Length="200" MinimumValue="0"
									SlideStep="1" />
							</div>
						</td>
						<td>
							<input id="currentWidthInput" type="text" readonly="readonly" style="width: 40px;
								text-align: right;" />
							px
						</td>
					</tr>
					<tr>
						...
						<td>
							<div style="margin-top: 4px;">
								<telerik:RadSlider ID="heightSlider" runat="server" Length="200" MinimumValue="0"
									SlideStep="1" />
							</div>
						</td>
						<td>
							<input id="currentHeightInput" type="text" readonly="readonly" style="width: 40px;
								text-align: right;" />
							px
						</td>
					</tr>
			...
			<fieldset style="width: 322px;">
				...
				<table cellpadding="0" cellspacing="0" style="width: 100%;">
					<tr>
						<td>
							<label for="CropX" style="margin-right: 4px;">
								x: <input disabled="disabled" type="text" id="CropX" style="width: 40px;" /> px
							</label>
						</td>
						<td>
							<script type="text/javascript">
							...
							<input disabled="disabled" type="text" id="CropWidth" style="margin-left: 5px; width: 40px;" /> px
						</td>
					...
					<tr>
						<td>
							<label for="CropX" style="margin-right: 4px;">
								y: <input disabled="disabled" type="text" id="CropY" style="width: 40px;" /> px</label>
						</td>
						<td>
							<label for="CropX" style="margin-right: 10px;">
								<script type="text/javascript">
								document.write(localization["Height"]);
								</script>:
								<input disabled="disabled" type="text" id="CropHeight" style="width: 40px;" /> px
							</label>
						</td>
					</tr>
				</table>
				...
				<tr>
					<td>
						...
						<div style="padding: 4px 0">
							...
							<input type="text" id="NewImageName" name="NewImageName" style="margin-left: 4px; width: 260px;" />
						</div>
					</td>
				</tr>
				<tr>
					<td>
						<div class="rade_InfoLabel" style="width: 315px;">
			...
			<table cellpadding="0" cellspacing="0" style="width: 100%;">
				<tr>
					<td class="rade_bottomcell" style="padding-top: 20px;">
						<table border="0">
							<tr>
